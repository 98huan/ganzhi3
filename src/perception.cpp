...
// #include "/home/zh/catkin_ws/src/xianggang_project/ganzhi3/include/ganzhi3/RsPointXYZIRT.h" //定义了速腾的点云结构，头文件放在devel/include中
// #include "/home/zh/catkin_ws/src/xianggang_project/ganzhi3/include/ganzhi3/eul2quat.h"
...
//         ofs4 << cloud_msg->pose.orientation.w << "\t" << cloud_msg->pose.orientation.x << "\t" << cloud_msg->pose.orientation.y << "\t" << cloud_msg->pose.orientation.z << endl;
...
const float radius = 0.2;           //小邻域搜索半径
const float radiusplus = 0.5;       //大邻域搜索半径
...
void lidar_rawpoint_sub_cb(const sensor_msgs::PointCloud2ConstPtr &cloud_msg1);
void xiaolinyu_sub_to_nihe(const sensor_msgs::PointCloud2ConstPtr &cloud_msg);
void plane_para_sub_cb(const ganzhi3::plane::ConstPtr &plane);
void kdtreeplus_subscriber_cb(const sensor_msgs::PointCloud2ConstPtr &cloud_msg);
...
int main(int argc, char **argv)
{
	...
	ros::Publisher plane_para_publisher_ = nh.advertise<ganzhi3::plane>("/plane_para", 10); 
        ros::Publisher transform_pub = nh.advertise<ganzhi3::guagou_in_houzhou>("/transform", 10);
...
void camera_sub_cb(const geometry_msgs::PoseStampedConstPtr &cloud_msg2)
{
        center_cam_x = (cloud_msg2->pose.position.z) + err_x;  // 相机的z方向是雷达的x方向
        center_cam_y = -(cloud_msg2->pose.position.x) + err_y; //相机的-x方向是雷达的+y方向
